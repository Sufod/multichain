...
// Version of bitcoinz transactions supported by the multichain.
...
func DefaultClientOptions() ClientOptions {
	return bitcoin.DefaultClientOptions().WithHost("http://127.0.0.1:11337")
}
...
// the Bitcoin Compat API, and exposes the functionality to build simple bitcoinz
// transactions.
...
// BuildTx returns a simple bitcoinz transaction that consumes the funds from the
...
// Tx represents a simple bitcoinz transaction that implements the Bitcoin Compat
// API.
type Tx struct {
	inputs     []utxo.Input
...
func calculateSighash(
	...
	// << GetHeader
	// First write out, then encode the transaction's nVersion number. bitcoinz current nVersion = 3
	var bVersion [4]byte
