...
var _ = Describe("bitcoinz", func() {
	Context("when submitting transactions", func() {
		Context("when sending BTCZ to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting bitcoinz_PK to
				// the value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("bitcoinz_PK")
				if pkEnv == "" {
					panic("bitcoinz_PK is undefined")
				}
				wif, err := btcutil.DecodeWIF(pkEnv)
				Expect(err).ToNot(HaveOccurred())

				// PKH
				pkhAddr, err := bitcoinz.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeCompressed()), &bitcoinz.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				pkhAddrUncompressed, err := bitcoinz.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeUncompressed()), &bitcoinz.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				...
				tx, err := bitcoinz.NewTxBuilder(&bitcoinz.RegressionNetParams, 1000000).BuildTx(inputs, recipients)
				Expect(err).ToNot(HaveOccurred())
				...
				// bitcoinz nodes are a little slow, so we wait for 1000 ms before
